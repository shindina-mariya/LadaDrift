...
$reviews = [];
try {
    $stmt = Database::query("SELECT * FROM reviews ORDER BY created_at DESC LIMIT 100");
...
<body>
    <aside class="sidebar">
        ...
        <a href="manage_reviews.php" class="sidebar__nav-item active"><i class="fas fa-star"></i> Отзывы</a>
        <a href="settings.php" class="sidebar__nav-item"><i class="fas fa-cog"></i> Настройки</a>
    </aside>
    <main class="main">
        <h1 style="margin-bottom:1.5rem;">Отзывы</h1>
        <?php if (empty($reviews)): ?>
        <div class="card">Отзывов пока нет.</div>
        <?php else: ?>
        <div class="card">
            <?php foreach ($reviews as $r): ?>
            <div style="text-align:left; padding:1rem 0; border-bottom:1px solid #333;">
                <strong><?= htmlspecialchars($r['client_name'] ?? '') ?></strong> — <?= (int)($r['rating'] ?? 0) ?> ★<br>
